...
def mk_set(dataset, concept_dict, tokenizer):
    ...
    with torch.no_grad():
        for i, id in enumerate(dataset.keys()):
            X[i] = model(tokenizer.encode(dataset[id]['mention'], padding="max_length", max_length=max_length, truncation=True, add_special_tokens = True, return_tensors="pt").to(device))[0][:,0]
            y[i] = model(tokenizer.encode(concept_dict[dataset[id]['cui'][0]]['label'], padding="max_length", max_length=max_length, truncation=True, add_special_tokens = True, return_tensors="pt").to(device))[0][:,0]
...
for epoch in range(epochs):
    for X, y in train_dataloader: # both X and y contains n=batch_size mentions and labels embeddings respectively
        batch_loss = None
        for mention, label in zip(X, y):
            pred = basenorm(mention) # Single linear layer
            ground_truth = basenorm(label)
        ...
        lr_scheduler.step()
        optimizer.zero_grad()
    print(f"Training: Epoch n° {epoch}, loss = {batch_loss.item()}")
