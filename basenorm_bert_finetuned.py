...
def inference():
    ...
    with torch.no_grad():
        for cui in tqdm(dd_ref.keys(), desc='Building embeddings from ontology labels'):
            ...
            dd_conceptVectors[cui][dd_ref[cui]["label"]] = model(tokenize(dd_ref[cui]['label']))[0][:,0].cpu().detach().numpy()# The last hidden-state is the first element of the output tuple
            nbLabtags += 1
            if embbed_size == None:
            ...
            if dd_ref[cui]["tags"]:
                for tag in dd_ref[cui]["tags"]:
                    nbLabtags += 1
                    dd_conceptVectors[cui][tag] = model(tokenize(tag))[0][:,0].cpu().detach().numpy()
...
for epoch in range(epochs):
    for X, y in train_dataloader: # both X and y contains n=batch_size tokenized mentions and labels respectively
        ...
        for tokenized_mention, tokenized_label in zip(X, y):
            ...
            pred = model(tokenized_mention)[0][:,0] # Taking last hidden state of the embedding model
